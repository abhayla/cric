...
void main() {
  ...
  group('enqueueUndo', () {
    test('inserts delete operation into sync queue', () async {
      ...
      await service.enqueueUndo(matchId: 'match-1', deliveryId: 'del-1');

      await Future<void>.delayed(const Duration(milliseconds: 50));

      verify(
        () => mockDio.delete('/api/v1/matches/match-1/deliveries/del-1'),
      ).called(1);
    });
  });
  ...
  group('startPeriodicSync', () {
    test('creates timer that calls processSyncQueue', () async {
      ...
      service.startPeriodicSync(interval: const Duration(milliseconds: 100));

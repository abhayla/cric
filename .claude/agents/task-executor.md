---
name: task-executor
description: Executes implementation completely self-contained following task files. Use when task files exist in docs/planning/plans/tasks/, or when "execute task/implement task/start implementation" is mentioned. Asks no questions, executes consistently from investigation to implementation.
...
1. **Task Execution**
   - Read and execute task files from `docs/planning/plans/tasks/`
...
Select and execute files with pattern `docs/planning/plans/tasks/*-task-*.md` that have uncompleted checkboxes `[ ]` remaining
...
#### Code Generation
**Code Generation:** After modifying files with Freezed annotations (`@freezed`) or Drift table definitions, run `dart run build_runner build --delete-conflicting-outputs` before running tests. For Drizzle schema changes on the server, run `bunx drizzle-kit generate && bunx drizzle-kit migrate`.
...
Examples: `docs/planning/plans/analysis/research-results.md`, `docs/planning/plans/analysis/api-spec.md`

...
description: Reads work plan documents from docs/planning/plans and decomposes them into independent, single-commit granularity tasks placed in docs/planning/plans/tasks. PROACTIVELY proposes task decomposition when work plans are created.
...
1. **Work Plan Analysis**
   - Load work plans from `docs/planning/plans/`
...
3. **Task File Generation**
   - Create individual task files in `docs/planning/plans/tasks/`
...
1. **Plan Selection**
   ...
   ```bash
   ls docs/planning/plans/*.md | grep -v template.md
   ```
...
3. **Overall Design Document Creation**
   - Record overall design in `docs/planning/plans/tasks/_overview-{plan-name}.md`
...
4. **Task File Generation**
   ...
   - Layer-aware naming (when the plan spans multiple layers): `{plan-name}-server-task-{number}.md`, `{plan-name}-mobile-task-{number}.md`
   - Layer is determined from the task's Target files paths
   - Examples: `20250122-refactor-types-task-01.md`, `20250122-auth-server-task-01.md`, `20250122-auth-mobile-task-02.md`
...
```
Task 1: [Content] → Deliverable: docs/planning/plans/analysis/[filename]
  ↓
Task 2: [Content] → Deliverable: docs/planning/plans/analysis/[filename]

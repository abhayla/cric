...
**Build Prerequisites:** Tasks that modify `data/models/` or `shared/data/database/` in Flutter require a code generation step (`dart run build_runner build`). Tasks that modify `db/schema/` on the server require Drizzle migration (`bunx drizzle-kit generate && bunx drizzle-kit migrate`). Include these steps in affected task descriptions.

...
GridView.builder(
  gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
    crossAxisCount: 2, // Phone-only: fixed 2-column grid
    crossAxisSpacing: 16,
    mainAxisSpacing: 16,
...
### Push with Result (go_router)

```dart
// Push and wait for result
final result = await context.push<bool>('/details/${item.id}');
...
### Route with Path Parameters (go_router)

```dart
// Define route in GoRouter config
GoRoute(
  path: '/details/:id',
...
class PhoneLayout extends StatelessWidget {
  ...
  const PhoneLayout({required this.child});

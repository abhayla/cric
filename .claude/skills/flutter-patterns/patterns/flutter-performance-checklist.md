...
- [ ] Using ref.watch(provider.select(...)) for specific field updates
- [ ] No setState called in loops
- [ ] Heavy computations moved outside build()

```dart
// ✅ Good - Watch entire provider (rebuilds on any state change)
Consumer(
  builder: (context, ref, child) => Text('${ref.watch(cartProvider).itemCount}'),
)

// ✅ Better - Only rebuild on itemCount change using select
Consumer(
